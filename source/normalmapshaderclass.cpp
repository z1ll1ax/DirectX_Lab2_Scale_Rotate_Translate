...
bool NormalMapShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	error = wcscpy_s(vsFilename, 128, L"C:\\Users\\Владислав\\Desktop\\laba\\source\\normalmap.vs");
	if(error != 0)
	{
		return false;
	}
	...
	error = wcscpy_s(psFilename, 128, L"C:\\Users\\Владислав\\Desktop\\laba\\source\\normalmap.ps");
	if(error != 0)
	{
		return false;
	}
...
bool NormalMapShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	if(FAILED(result))
	{
		...
		{
			MessageBox(hwnd, "../source/normalmap.vs", "Missing Shader File", MB_OK);
		}
	...
	if(FAILED(result))
	{
		...
		{
			MessageBox(hwnd, "../source/normalmap.ps", "Missing Shader File", MB_OK);
		}
